...
export function setCloudServeFn(app) {
  ...
  // 创建批量任务
  app.post(
    "/cloud/serve/createBatchTask",
    authenticateToken,
    async (req, res) => {
      CloudAPI.createBatchTask(req.body, req.headers.cloud)
        .then((el) => {
          CloudAPI.checkBatchTaskFn({
            taskId: el.taskId,
            type: req.body.type,
            cookie: req.headers.cloud,
            result: res,
          });
        })
        .catch((err) => {
  ...
  //  获取分享信息
  app.post(
    "/cloud/serve/getShareInfoByCode",
    authenticateToken,
    async (req, res) => {
      CloudAPI.getShareInfoByCode(
        req.body.shareCode,
        req.body.accessCode,
        req.headers.cloud,
        res,
  ...
  // 获取下级目录节点
  app.post(
    "/cloud/serve/getObjectFolderNodes",
    authenticateToken,
    async (req, res) => {
  ...
  app.post("/cloud/serve/getFullPath", authenticateToken, async (req, res) => {
    CloudAPI.getFullPath(req.body.fileId, req.headers.cloud, res).then((el) => {
      res
        .status(200)
        .json({ res_code: 0, res_message: "获取成功", data: { ...el } });
    });
  });
  // 回收站
  app.post(
    "/cloud/serve/getRecycleList",
    authenticateToken,
    async (req, res) => {
