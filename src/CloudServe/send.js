...
function getShareInfoByCode(shareCode, accessCode = "", cookie, result) {
    axios
        ...
        .then((res) => {
            ...
            if (accessCode && res.needAccessCode == 1) {
                ...
                axios
                    ...
                    .then((checkRes) => {
                        ...
                        result && result.status(200).json({ ...res, shareId: checkRes.shareId })
                    })
                    .catch((checkerr) => {
...
async function getListShareDir(params, cookie, result) {
    try {
        const res = await axios.get(`https://cloud.189.cn/api/open/share/listShareDir.action?${qs.stringify(params)}`, {
            headers: {
                "content-type": "application/x-www-form-urlencoded",
                accept: "application/json;charset=UTF-8",
                cookie: cookie,
