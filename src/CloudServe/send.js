...
function getUserSizeInfo(cookie, result) {
  ...
  axios
    .get(
      `https://cloud.189.cn/api/portal/getUserSizeInfo.action?noCache=${Math.random()}`,
      {
        headers: {
          accept: "application/json;charset=UTF-8",
          "sign-type": "1",
...
async function renameFolder(params, cookie, result) {
  ...
  const data = {
    ...
    destFolderName: params.destFolderName,
  };
  axios
...
//  获取分享链接信息
function getShareInfoByCode(shareCode, accessCode = "", cookie, result) {
  axios
    .get(
      `https://cloud.189.cn/api/open/share/getShareInfoByCodeV2.action?shareCode=${shareCode}`,
      {
        headers: {
          "content-type": "application/x-www-form-urlencoded",
          accept: "application/json;charset=UTF-8",
    ...
    .then((res) => {
      //   console.log("getShareInfoByCode获取分享链接信息 >>>>>:", res);
      if (res.needAccessCode == 1) {
        let noCache = Math.random();
        axios
          .get(
            `https://cloud.189.cn/api/open/share/checkAccessCode.action?noCache=${noCache}&shareCode=${shareCode}&accessCode=${accessCode}`,
            {
              headers: {
                "content-type": "application/x-www-form-urlencoded",
                accept: "application/json;charset=UTF-8",
          ...
          .then((checkRes) => {
            // console.log("checkAccessCode校验提取码 >>>>>:", checkRes);
            result &&
              result.status(200).json({ ...res, shareId: checkRes.shareId });
          })
          .catch((checkerr) => {
            handleError(checkerr, result, "获取分享链接信息 checkAccessCode");
          });
      } else {
        result && result.status(200).json(res);
      }
...
async function getListShareDir(params, cookie, result) {
  try {
    const res = await axios.get(
      `https://cloud.189.cn/api/open/share/listShareDir.action?${qs.stringify(params)}`,
      {
        headers: {
          "content-type": "application/x-www-form-urlencoded",
          accept: "application/json;charset=UTF-8",
